	...
	<h3 id="existing_puzzles">Ongoing Puzzles</h3>
		...
		echo '<div class="span4 scrollable_div ong_container" id="medium_section"><h3 class="instructions">Medium</h3>';
			for($a=0; $a<sizeof($medium); $a++){
				...
				$datediff = $now - $your_date;
				$datediff= time_elapsed($datediff);
				echo '<br/><div class="ongoing_puzzle"><img src ="'.$var["name"].'.png " class="exist_puz_photos img-polariod" alt="' . $var["name"] . '" /><br/><p class="time_elapsed">Time Elapsed: '.$datediff.'</p><p class="participating_friends">Puzzle Players: '.$string_of_players.'</p><button class="resume_puzzle btn btn-mini" type="submit" name="' . $var["id"] .'_25_'.$var["name"].'""><i class="icon-repeat"></i> Resume</button><button class="start_puzzle_over btn btn-mini" type="submit" name="' . $var["id"] . '"/><i class="icon-remove"></i> Give Up</button></div>';
